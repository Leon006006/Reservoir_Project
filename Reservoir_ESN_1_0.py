...
class ESN_Reservoir(object):
	...
	def train(self, Input, desired_Output):
		...
		if self.Reservoir.feedback == 1:
			from_feedback = np.dot(self.Reservoir.W_feedback, desired_Output[:, 0].reshape(desired_Output.shape[0], 1))
			self.Neuron_State[:, 0] = np.tanh(from_intern + from_input + 0.2 * from_feedback).reshape(self.Reservoir.Number_Neurons, )
		...
		for steppystep in range(1, self.steps):
			if self.Reservoir.feedback == 1:
				self.Neuron_State[:, steppystep] = np.tanh(np.dot(self.Reservoir.W_intern, self.Neuron_State[:, steppystep-1]) + np.dot(self.Reservoir.W_input, Input[:, steppystep]) + 0.2 * np.dot(self.Reservoir.W_feedback, desired_Output[:, steppystep-1]))
			elif self.Reservoir.feedback == 0:
			...
			if steppystep % 10 == 0: print('\rTraining-Progress: {}%'.format(np.round((steppystep+1)/self.steps * 100.), 2), end='')

		self.ReadOut = (np.dot(linalg.pinv(self.z.T), desired_Output.T))
		self.ReadOut = self.ReadOut.T
	...
	def guess(self, time_Vec):
		...
		if self.Reservoir.feedback == 1:
			...
			current_Neuron_State = np.tanh(np.add(np.add(from_intern, from_input), 0.2*from_feedback))
		elif self.Reservoir.feedback == 0:
